"""
نظام إدارة حسابات الحج والعمرة
تطبيق Tkinter محسّن مع ممارسات أفضل وإدارة أفضل للموارد
"""
import csv
import html
import os
...
def init_db() -> None:
    """تهيئة قاعدة البيانات مع التحقق من الأعمدة"""
    with get_db() as connection:
        ...
        try:
            ...
            logger.warning(f"الفهارس موجودة بالفعل: {e}")
...
class App(tk.Tk):
    ...
    def build_shell(self) -> None:
        ...
        tk.Label(header, text="رياحين طيبة", bg=self.colors["navy"], fg="white",
                font=("Segoe UI", 22, "bold")).pack(side="right", padx=28, pady=8)
    ...
    def card(self, parent: tk.Widget, title: str, value: float, currency: str, color: str) -> tk.Frame:
        ...
        tk.Label(inner, text=money(value, currency), bg="white", fg=self.colors["navy"],
                font=("Segoe UI", 17, "bold")).pack(anchor="e", pady=7)
        return frame
